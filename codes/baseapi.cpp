...
int TessBaseAPI::Recognize(ETEXT_DESC* monitor) {
  ...
  if (tesseract_->interactive_mode) {
    ...
    //std::cout<<"Now run the main recognition"<<std::endl;
    // writing block line word cordinates by kvJ
     PAGE_RES_IT page_res_it;
     page_res_it.page_res = page_res_;
     
     std::string inputFileName (input_file_->string());
      //===========================================================================================================================
 
 std::ofstream wordCo;    //create output file
 std::string wordN (".words.txt");
 std::string wordFn = inputFileName+wordN;
 wordCo.open(wordFn.c_str(),std::ios::app);
//--------------------------------------------------
 std::ofstream lineCo;    //create output file
 std::string lineN (".lines.txt");
 std::string lineFn = inputFileName+lineN;
 lineCo.open(lineFn.c_str(),std::ios::app);
 int x1=0,y1=0;
//---------------------------------------------------
 std::ofstream blockCo;    //create output file
 std::string blockN (".blocks.txt");
 std::string blockFn = inputFileName+blockN;  
 blockCo.open(blockFn.c_str(),std::ios::app);
 int x2=0,y2=0,h2=0;
//--------------------------------------------------	
  page_res_it.restart_page();
  while ( page_res_it.word() != NULL)
  { 
    // write word cordinates
       wordCo<<page_res_it.word()->word->bounding_box().left()<<"\t"<<height_jobs-page_res_it.word()->word->bounding_box().top()<<"\t"<<page_res_it.word()->word->bounding_box().width()<<"\t"<<page_res_it.word()->word->bounding_box().height()<<"\n";  
 
    // write line
    if (x1!=page_res_it.row()->row->bounding_box().left() or y1!=page_res_it.row()->row->bounding_box().top())
      lineCo<<page_res_it.row()->row->bounding_box().left()<<"\t"<<height_jobs-page_res_it.row()->row->bounding_box().top()<<"\t"<<page_res_it.row()->row->bounding_box().width()<<"\t"<<page_res_it.row()->row->bounding_box().height()<<"\n";
    x1=page_res_it.row()->row->bounding_box().left(); y1=page_res_it.row()->row->bounding_box().top();

    //write block
    if (x2!=page_res_it.block()->block->bounding_box().left() or y2!=page_res_it.block()->block->bounding_box().top() or h2 != page_res_it.block()->block->bounding_box().height())
        blockCo<<page_res_it.block()->block->bounding_box().left()<<"\t"<<height_jobs-page_res_it.block()->block->bounding_box().top()<<"\t"<<page_res_it.block()->block->bounding_box().width()<<"\t"<<page_res_it.block()->block->bounding_box().height()<<"\n";
        x2=page_res_it.block()->block->bounding_box().left(); y2=page_res_it.block()->block->bounding_box().top();
        h2= page_res_it.block()->block->bounding_box().height();
  ...
  //=================================================================================================================================     
    ...
    //if (tesseract_->recog_all_words(page_res_, monitor, NULL, NULL, 0)) {
      //  std::cout<<"recog_all_words "<<std::endl;
...
bool TessBaseAPI::ProcessPage(Pix* pix, int page_index, const char* filename,
  ...
  //if (tesseract_->tessedit_write_images) {
    ...
    std::string inputFile (input_file_->string()),appnd ("bin_");
    std::string newName = appnd+inputFile;
    pixWrite(newName.c_str(), page_pix, IFF_TIFF_G4);// writing tiff input image  
   // pixWrite("BinImg.tif", page_pix, IFF_TIFF_G4);
  //}
